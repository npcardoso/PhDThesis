...
t_json_observation_serializer::t_json_observation_serializer () {
    static t_ptr<t_element_group_writer> def_group(new t_json_array());


    group = def_group;
}

t_json_observation_serializer::t_json_observation_serializer (t_ptr<t_element_group_writer> group) : group(group) {}
...
bool t_json_observation_serializer::operator () (std::ostream & out,
    ...
    t_ptr<t_element_group_writer> arr_group(new t_json_array());
    ...
    if (obs->oracles.size()) {
        ...
        tmp.group = arr_group;
        BOOST_FOREACH(t_ptr<t_oracle_observation> o, obs->oracles)
        tmp(out, o);
        tmp.close(out);
    }
    ...
    if (obs->probes.size()) {
        ...
        t_json_observation_serializer tmp(*this);
        tmp.group = arr_group;
        BOOST_FOREACH(t_ptr<t_probe_observation> p, obs->probes)
        tmp(out, p);
        tmp.close(out);
    }
    ...
    if (obs->transactions.size()) {
        ...
        t_json_observation_serializer tmp(*this);
        tmp.group = arr_group;
        BOOST_FOREACH(t_ptr<t_transaction_observation> tr, obs->transactions)
        tmp(out, tr);
        tmp.close(out);
    }
...
t_ptr<t_observation_serializer> t_json_observation_serializer::array () {
    t_ptr<t_element_group_writer> tmp_group(new t_json_array());
    t_ptr<t_observation_serializer> tmp(new t_json_observation_serializer(tmp_group));
