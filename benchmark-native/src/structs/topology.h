...
public:
    t_fault ();
    t_fault (t_probability goodness,
    ...
    inline t_probability get_pass_prob () const {
        return pass_prob;
    }

    inline t_probability get_soft_prob () const {
        return soft_prob;
    }

    inline t_probability get_hard_prob () const {
        return hard_prob;
    }

    inline t_probability get_fail_prob () const {
        return fail_prob;
    }
...
public:
    ...
    t_link & add_sink (t_component_id comp,
                       t_probability prob);

    inline const t_sinks & get_sinks () const {
        return sinks;
    }

    t_probability get_probability (t_component_id comp) const;
    inline t_probability get_normalization_value () const {
        return total;
    }
...
public:
    ...
    t_topology & add_fault (t_component_id comp,
                            const t_fault & f);
    t_topology & add_link (t_component_id comp,
                           const t_link & l);
    t_topology & add_entry_point (t_component_id comp,
                                  t_probability prob);
...
private:
    std::ostream & graphviz_component (std::ostream & out,
                                       t_component_id comp) const;
    std::ostream & graphviz_links (std::ostream & out,
                                   t_component_id comp) const;
