...
public:
    t_system_repeater (t_ptr<t_system> system, t_count count);
...
private:
    t_count count;
    t_ptr<t_system> system;
};

class t_architecture_repeater : public t_architecture {
public:
    t_architecture_repeater (t_ptr<t_architecture> arch, t_count count, t_count spectra_count);

    virtual t_system * operator () (boost::random::mt19937 & gen);

private:
    t_count count, spectra_count;
    t_ptr<t_architecture> arch;
};
}
}
