...
t_link & t_link::add_sink (t_component_id comp,
                           t_probability prob) {
    assert(prob >= 0);
    assert(prob <= 1);
...
t_topology & t_topology::add_fault (t_component_id comp,
                                    const t_fault & f) {
    assert(comp != 0);
...
t_topology & t_topology::add_link (t_component_id comp,
                                   const t_link & l) {
    assert(comp != 0);
...
t_topology & t_topology::add_entry_point (t_component_id comp,
                                          t_probability prob) {
    assert(comp != 0);
    components.insert(comp);
...
std::ostream & t_topology::graphviz_component (std::ostream & out,
...
std::ostream & t_topology::graphviz_links (std::ostream & out,
                                           t_component_id comp) const {
    const t_interface * iface = get_interface(comp);
