...
_instrument_llvm_builder_c = SCons.Builder.Builder(
    action = SCons.Action.Action('$CLANG -S -emit-llvm -g $CFLAGS -I $CPPPATH -c $SOURCE -o $TARGET'),
    src_suffix = '$INSTR_SOURCE_SUFFIX_C',
    suffix = '$INSTR_LLVM_SUFFIX',
    single_source = 1)

_instrument_llvm_builder_cpp = SCons.Builder.Builder(
    action = SCons.Action.Action('$CLANGPP -S -emit-llvm -g $CFLAGS -I $CPPPATH -c $SOURCE -o $TARGET'),
    src_suffix = '$INSTR_SOURCE_SUFFIX_CPP',
    suffix = '$INSTR_LLVM_SUFFIX',
    single_source = 1)

_instrument_llvm_instrument_builder = SCons.Builder.Builder(
    action = SCons.Action.Action('$OPT -S -f --load $LLVM_MODULE $LLVM_INSTRUMENT_PASSES $SOURCE -o $TARGET'),
...
def Instrument(env, target, source=None, passes="-spectrum_instrument", *args, **kw):
    ...
    llvm_instrument_lib = env.subst('$LLVM_MODULE')
